...
export const GetSessionID = createAsyncThunk(
  ...
  async (ipRange, thunkAPI) => {
    try {
      const response = await protectedApis.post("/v1/scan/start", {
        range: ipRange,
        severIP: "10.6.50.90",
      });
...
export const GetCheckStatus = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      ...
      const response = await protectedApis.post("/v1/scan/check", {
        sessionId,
      });
      const data = await response.data;
      if (response.status === 200) {
...
export const GetDeviceData = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const { sessionId } = thunkAPI.getState().deviceDiscovery;
      const response = await protectedApis.post("/v1/scan/result", {
        sessionId,
        page: 1,
        size: 20,
      });
...
export const GetlastScannedDeviceData = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await protectedApis.post("/v1/scan/result/last", {
        page: 1,
        size: 20,
      });
      const data = await response.data;
  ...
  extraReducers: {
    ...
    [GetCheckStatus.fulfilled]: (state, { payload }) => {
      ...
      return {
        ...
        sessionDetails: {
          ...state.sessionDetails,
          statusCheck: status === "running" ? "done" : "done",
          messageCheck: status,
        },
      };
    },
